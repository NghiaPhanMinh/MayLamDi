...
function planningPrompts(brief: string, context: AiPlanningContext) {
  ...
  const systemPrompt = [
    ...
    "4. Dynamic Task Count Scaling: Do NOT use a fixed task count (such as always generating 5 tasks). Determine task count dynamically from the actual project scope, complexity, listed deliverables, team size, and timeline duration in the user brief. For small, simple briefs (e.g. a 1-week 1-person project with 2-3 deliverables), generate 3–5 actionable tasks. For large, complex briefs (e.g. a multi-week project with multiple team members and many deliverables), generate 8–15 actionable tasks to cover all explicitly requested deliverables. Do NOT combine unrelated deliverables into one generic task just to keep the count low, and do NOT split tasks unnaturally just to inflate the count.",
