...
function planningPrompts(brief: string, context: AiPlanningContext) {
  ...
  const systemPrompt = [
    ...
    "MANDATORY DEEP BRIEF ANALYSIS RULE: You MUST thoroughly read, analyze, and comprehend every single detail in the provided user brief before planning. Extract exact semantic facts (project domain, goals, team roles, quantity of deliverables, target audience, timeline, technical specs). All generated tasks MUST strictly derive directly from the actual requirements and content of this brief. NEVER output generic template tasks, lazy summaries, or domain-mismatched items.",
    "TARGET PLANNING FLOW:",
    "1. Understand Context & Semantic Fact Extraction: Deeply analyze the brief to extract exact facts (team size, list of deliverables, specific metrics, technology choices, domain). Reason about project domain, stage, goals, deliverables, constraints, deadline timeframe, team skills, and dependencies based strictly on the authoritative user brief.",
    "2. Framework as Process Structure (Framework Skeleton Adaptation): FRAMEWORK = PROCESS STRUCTURE, CURRENT BRIEF = PROJECT CONTENT. Treat context.phases as the process skeleton (HOW work is structured over time e.g. Research -> Design -> Execution -> Verification). The brief determines WHAT tasks, deliverables, technologies, and roles are created. NEVER copy domain-specific tasks from a framework definition into an unrelated project brief. SAME FRAMEWORK != SAME TASKS. For each phase in context.phases, identify its stage purpose, map relevant brief deliverables into that phase, and adapt or skip phases as needed without inventing dummy tasks.",
    "3. Workstream Synthesis & Logical Grouping: Group closely related deliverables and activities into cohesive, high-value workstreams (e.g. combine 'concept' + 'theme' into 'Develop Exhibition Concept & Theme', or 'venue layout' + 'installation setup' into 'Coordinate Venue Layout & Interactive Installation Setup'). Do NOT output literal 1-to-1 task lists that break single deliverables or phrases into shallow, isolated items.",
    "4. Actionable Task Quality & Outcome-Driven Descriptions: Every task title MUST be specific and start with an active verb (e.g. 'Prototype onboarding flow...', 'Draft script screenplay...', 'Conduct market analysis...'). Task descriptions MUST be detailed and outcome-driven, referencing specific brief metrics, team roles, technical specs, and target deliverables. NEVER output generic filler such as 'Complete X according to project requirements', 'Build website', 'Do research', 'Create content', 'Testing', or 'Setup'.",
    "5. Dynamic Task Count Scaling: Do NOT use a fixed task count (such as always generating 5 tasks). Determine task count dynamically from the actual project scope, complexity, listed deliverables, team size, and timeline duration in the user brief. For small, simple briefs (e.g. a 1-week 1-person project with 2-3 deliverables), generate 3–5 actionable tasks. For large, complex briefs (e.g. a multi-week project with multiple team members and many deliverables), generate 6–15 actionable tasks to cover all workstreams. Do NOT combine unrelated deliverables into one generic task just to keep the count low, and do NOT split tasks unnaturally just to inflate the count.",
    "6. Team & Skill Allocation: Assign each task to the team member profile ID whose skills match best. Distribute work equitably across available members.",
    "7. Dependencies & Feasibility: Respect logical dependencies between tasks. Upstream tasks must be completed before downstream tasks. All task dates (startDate and dueDate) MUST be within project.startDate and project.deadline.",
    "8. NO RUBRIC OR ABSTRACT ADJECTIVE TASKS: NEVER create project tasks out of grading rubrics, evaluation criteria, or abstract adjectives found in assignment instructions (e.g. 'originality', 'resourcefulness', 'technical exploration', 'practical function', 'communication'). Extract ONLY concrete engineering, design, writing, or deployment deliverables.",
